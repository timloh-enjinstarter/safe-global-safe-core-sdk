...
} from '@timloh-enjinstarter/safe-global-safe-ethers-lib/typechain/src/ethers-v5/v1.3.0/'
import {
  ERC20Mintable,
} from '@timloh-enjinstarter/safe-global-safe-ethers-lib/typechain/tests/ethers-v5'
...
export const getSafeSingleton = async (): Promise<{
  ...
  return {
    contract: Safe.attach(SafeDeployment.address) as GnosisSafe_V1_3_0,
    abi: SafeDeployment.abi
  }
}

export const getFactory = async (): Promise<{
  contract: ProxyFactory_V1_3_0
  abi: AbiItem | AbiItem[]
}> => {
  ...
  return {
    contract: Factory.attach(FactoryDeployment.address) as ProxyFactory_V1_3_0,
...
> => {
  ...
  const Safe = await ethers.getContractFactory(gnosisSafeDeployed.name)
  return Safe.attach(template) as GnosisSafe_V1_3_0
}

export const getSafeWithOwners = async (
  owners: string[],
  threshold?: number
): Promise<GnosisSafe_V1_3_0> => {
  ...
  return template as GnosisSafe_V1_3_0
}

export const getMultiSend = async (): Promise<{
  contract: MultiSend_V1_3_0
  abi: AbiItem | AbiItem[]
}> => {
  ...
  return {
    contract: MultiSend.attach(MultiSendDeployment.address) as MultiSend_V1_3_0,
